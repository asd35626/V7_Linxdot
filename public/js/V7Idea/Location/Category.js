  ...
  function toAdd() {
    ...
    if (id == '') {
      window.location = "/Location/CategoryAdd";
    } else {
      window.location = "/Location/CategoryAdd/" + id;
    }
  }
  ...
  function editData(thisId) {
    ...
    window.location = "/Location/CategoryEdit/" + thisId;
  }

  function deleteData(thisId) {
    if (confirm("確定要刪除嗎?")) {
      $.ajax({
        url: "/api/v1/LocationCategory/" + thisId,
        type: 'DELETE',
        headers: {
  ...
  function reload(thisId) {
    ...
    window.location = "/Location/Category/" + thisId;
