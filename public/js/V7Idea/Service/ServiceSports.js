...
function listWebDocumentTable() {
  var table = $('#grid-basic').DataTable({
    ...
    "ajax": {
      ...
      "data": function(d) {
        d.action = "getNewsList";
        d.searchTitle = "SportsCategory";
        d.searchDocument = $('#list #DocumentTitle').val();
      }
    },
    "columns": [{
      "data": "SerialNO",
...
function toAdd() {
  ...
  $('#edit').hide();
  $('#add').show();
  loadCategoryDCID('#addForm #addDCID');
}
  ...
  $.ajax({
    ...
    success: function(response) {
      if (response.status == 1) {
        ...
        $('#editShortDescription').val(response.data.ShortDescription);
        ...
        //
        $('#editShowImage1').empty();
        if ((response.data.OtherImagePath1 == '' || response.data.OtherImagePath1 == undefined));
        else $('#editShowImage1').prepend('<img src="/Service/' + response.data.OtherImagePath1 + '" />');
      }
    },
...
function loadCategoryDCID(selectListId) {
  $.ajax({
    url: "/api/v1/WebDocumentCategory?searchTitle=SportsCategory&length=-1",
    type: 'GET',
    async: false,
    headers: {
      'Authorization': Cookies.get('authToken')
