...
function listCategories() {
  var table = $('#grid-basic').DataTable({
    ...
    "ajax": {
      "url": "/api/v1/WebDirectVideoCategory",
      "type": "POST",
...
function toAdd() {
  ...
  if (id == '') {
    window.location = "/DirectVideo/CategoryAdd";
  } else {
    window.location = "/DirectVideo/CategoryAdd/" + id;
  }
}
...
function editData(thisId) {
  ...
  window.location = "/DirectVideo/CategoryEdit/" + thisId;
}

function deleteData(thisId) {
  if (confirm("確定要刪除嗎?")) {
    $.ajax({
      url: "/api/v1/WebDirectVideoCategory/" + thisId,
      type: 'DELETE',
      headers: {
...
function reload(thisId) {
  ...
  window.location = "/DirectVideo/Category/" + thisId;
