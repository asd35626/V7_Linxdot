  ...
  var table = $('#grid-basic').DataTable({
    ...
    "ajax": {
      ...
      "data": function(d) {
        d.action = "list";
        d.searchUserType = 20;
        d.searchCompanyName = $('#searchCompanyName').val();
        d.searchCompanyNo = $('#searchCompanyNo').val();
      }
    },
    "columns": [{
      "data": "CompanyName"
  ...
  $.ajax({
    ...
    success: function(response) {
      if (response.status == 1) {
        ...
        $('#editCompanyNo').val(response.data.CompanyNo);
        $('#editCompanyPhone').val(response.data.CompanyPhone);
        $('#editCompanyFax').val(response.data.CompanyFax);
        $('#editCompanyAddress').val(response.data.CompanyAddress);
        $('#editCompanyCapital').val(response.data.CompanyCapital);
        $('#editCompanyEmail').val(response.data.CompanyEmail);
        $('#editUserMobile').val(response.data.UserMobile);
        $('#editRealName').val(response.data.RealName);
        $('#editInvoiceAddress').val(response.data.InvoiceAddress);
        $('#editProductDescription').val(response.data.ProductDescription);
        ...
        if (response.data.SignDate) $('#editSignDate').val(response.data.SignDate);
        else $('#editSignDate').val('');
        if (response.data.CompanyStartDate) $('#editCompanyStartDate').val(response.data.CompanyStartDate);
        else $('#editCompanyStartDate').val('');
...
function loadDegreedList(selectListId) {
  $.ajax({
    url: "/api/v1/DimUserDegreeToUserType?userType=20&length=-1",
    type: 'GET',
    async: false,
    headers: {
      'Authorization': Cookies.get('authToken')
