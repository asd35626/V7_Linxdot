  ...
  function listCategories() {
    var table = $('#grid-basic').DataTable({
      ...
      "ajax": {
        ...
        "data": function(d) {
          d.action = "list";
          d.searchCategoryTitle = $('#searchCategoryName').val();
  ...
  function toAdd() {
    ...
    if (id == '') {
      window.location = "/Activity/CategoryAdd";
    } else {
      window.location = "/Activity/CategoryAdd/" + id;
    }
  }
  ...
  function editData(thisId) {
    ...
    window.location = "/Activity/CategoryEdit/" + thisId;
  }

  function deleteData(thisId) {
    if (confirm("確定要刪除嗎?")) {
      $.ajax({
        url: "/api/v1/ActivityCategory/" + thisId,
        type: 'DELETE',
        headers: {
  ...
  function reload(thisId) {
    ...
    window.location = "/Activity/Category/" + thisId;
