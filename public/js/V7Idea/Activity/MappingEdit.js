  ...
  $(document).ready(function() {
    ...
    if (id == undefined) {
      ...
      //strp 2 : load Activity list
      loadActivityList('#editBrandID');
  ...
  function loadActivityList(selectListId) {
    $.ajax({
    ...
    $.ajax({
      ...
      success: function(response) {
        if (response.status == 1) {
          ...
          $('#editForm #editBrandID').val(response.data.ActivityID);
  ...
  function sendEditData() {
    ...
    if(!isSuccess){
      ...
      $.ajax({
        url: "/api/v1/ActivityMapping/" + $('#editId').val(),
        type: 'PUT',
        data: formData,
        async: false,
        headers: {
