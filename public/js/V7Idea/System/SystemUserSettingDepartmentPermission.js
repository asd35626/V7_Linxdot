...
function listAdminDegreeFunctions() {
  var table = $('#grid-basic').DataTable({
    ...
    "columnDefs": [{
      ...
      "targets": 4,
      "render": function(data, type, row) {
        return "<div class=\"btn btn-sm btn-primary\" onclick=\"showDetail('" + row.DegreeId + "','" + row.DegreeName + "');\">瀏覽</div>";
      }
    }],
    "bSort": false, //排序
    "bFilter": false, //搜尋
...
//showDetail
function showDetail(degreeId, degreeName) {
...
function listAdminDetailTable() {
  var table = $('#grid-basic-detail').DataTable({
    ...
    "columnDefs": [{
      "targets": 3,
      "render": function(data, type, row) {
        if (row.IfAccess == 1) return "<div class=\"btn btn-sm btn-danger\" onclick=\"turnOff('" + row.PermissionId + "','" + row.FunctionId + "');\">開啟中</div>";
        else return "<div class=\"btn btn-sm btn-primary\" onclick=\"turnOn('" + row.PermissionId + "','" + row.FunctionId + "');\">已關閉</div>";
      }
    }],
    "bSort": false, //排序
    "bFilter": false, //搜尋
