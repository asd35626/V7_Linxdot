                ...
                <table id="grid-basic" class="uk-table uk-table-nowrap table_check">
                    <thead>
                        <tr>
                            ...
                            <th class="uk-width-1-10 uk-text-small">類別代碼</th>
                            <th class="uk-width-2-10 uk-text-small">類別名稱</th>
                            <th class="uk-width-1-10 uk-text-small">標記</th>
                            <th class="uk-width-2-10 uk-text-small">備註</th>
                            <th class="uk-width-2-10 uk-text-small">啟用/停用</th>
                    ...
                    @if($data->count() > 0)
                        @foreach ($data as $object)
                            <tr>
                                ...
                                <td class="uk-text-small">{{ $object->UserTypeId}}</td>
                                <td class="uk-text-small">{{ $object->UserTypeName}}</td>
                                <td class="uk-text-small">{{ $object->UserTypeMark }}</td>
                                <td class="uk-text-small">{{ $object->UserTypeMemo }}</td>
                                ...
                                <td class="uk-text-center uk-text-small">
                                    <a href="{{ route($routePath.'.edit',$object->UserTypeId) }}"><i class="md-icon material-icons">&#xE254;</i></a>
                                    <!-- {!! Form::open(['id' => 'formDeleteAction'.$i , 'method' => 'DELETE','route' => [ $routePath.'.destroy', $object->UserTypeId],'style'=>'display:inline']  ) !!}
